import type { DutyCalculationInput, DutyCalculationResult, ComplianceStatus } from '@/types/compliance';
import { CREW_DUTY_LIMITS } from './regulations';

/**
 * Calculate crew duty compliance for proposed flight
 * Based on 14 CFR § 135.267
 */
export function calculateCrewDuty(input: DutyCalculationInput): DutyCalculationResult {
  ...
  const limit24Hour = crewSize === 1 
    ? CREW_DUTY_LIMITS.singlePilot24Hour 
    : CREW_DUTY_LIMITS.twoPilot24Hour;
  ...
  if (totalWithProposed <= limit24Hour) {
    ...
    // Provide solutions
    if (crewSize === 1) {
      const neededWithTwoPilot = totalWithProposed - CREW_DUTY_LIMITS.twoPilot24Hour;
      if (neededWithTwoPilot <= 0) {
        suggestions.push(`Use two-pilot crew (allows ${CREW_DUTY_LIMITS.twoPilot24Hour} hours, making this legal)`);
      }
    }
...
/**
 * Calculate how many hours pilot has remaining in current 24-hour period
 */
export function getRemainingDutyHours(
  recentFlights: { date: string; hours: number }[],
  crewSize: 1 | 2
): number {
  const limit = crewSize === 1 
    ? CREW_DUTY_LIMITS.singlePilot24Hour 
    : CREW_DUTY_LIMITS.twoPilot24Hour;
  ...
  recentFlights.forEach(flight => {
    const flightDate = new Date(flight.date);
    if (flightDate >= twentyFourHoursAgo) {
      hoursUsed += flight.hours;
    }
  });

  return Math.max(0, limit - hoursUsed);
}
