// METAR API Route - Server-side proxy for CheckWX METAR data
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.allowed) {
      logApiUsage("metar", clientId, undefined, false, "Rate limit exceeded");
    ...
    if (!icaoResult.success) {
      logApiUsage("metar", clientId, undefined, false, icaoResult.error);
    ...
    const metarData = await client.getMetar(icaos);

    // Log successful usage
    logApiUsage("metar", clientId, icaos, true);
    ...
    logApiUsage("metar", clientId, undefined, false, errorMessage);
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Internal server error occurred while fetching METAR data",
        data: [],
        timestamp: new Date().toISOString(),
      },
      {
